...
class EpicParser():
    ...
    def parse_problem_list(self, xml_prob_list, patient_enc, pat):

        problem_list = []
        probl_root = ET.fromstring(xml_prob_list)
        try:
            for prb in probl_root.findall(".//Problems/Problem"):
                condition = parse_medical_problem(prb, patient_enc)
    ...
    def parse_orders(self, str, pat_enc):
        #raise NotImplementedError
        encounter_orders = []
...
def parse_medical_problem(xmlIn, enc):

    prob = api.Condition(encounter=enc)
    root = xmlIn
...
if __name__ == '__main__':
    ...
    EP_ord = EpicParser().parse_orders("<Orders>    <Order>          <ProcedureCode>85025</ProcedureCode>          <CodeType>CPT</CodeType>          <ExpectedDate>2011-01-01T00:00:00</ExpectedDate>          <ExpiredDate></ExpiredDate>          <Name>CBC with Automated Diff</Name>          <Type>Lab</Type>   </Order></Orders>", EP_enc)