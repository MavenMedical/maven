    ...
    getClosestIndex: function (a, b, c, d, e, g) {
        ...
        if (b) {
            ...
            for (var n = 0; n < c.length; n++) {
                        ...
                        if (x.values) {
                            ...
                            if (!isNaN(C)) {
                                ...
                                var D = AmCharts.getDecimals(C),
                                    h = h + C, h = AmCharts.roundTo(h, Math.max(y, D));
                                f++;
                                y = h / f
                            }
                        }
                ...
                w = {open: q, close: t, high: z, low: v, average: A, sum: w, count: r};
                a = AmCharts.formatValue(a, {open: m, close: u, high: s, low: p, average: y, sum: h, count: f}, d, k, l + "\\.", this.usePrefixes, this.prefixesOfSmallNumbers, this.prefixesOfBigNumbers);
        ...
        if (-1 != a.indexOf("[[category]]")) {
            ...
            if (this.categoryAxis.parseDates) {
                var g = this.balloonDateFormat, k = this.chartCursor;
...
AmCharts.Cuboid = AmCharts.Class({construct: function (a, b, c, d, e, g, k, n, l, h, f, m, u, p, s) {
    this.set = a.set();
    this.container = a;
    this.h = Math.round(c);
    this.w = Math.round(b);
    ...
    var b = this.container, c = this.w, d = this.h, e = this.dx, g =
        this.dy, k = this.colors, n = this.alpha, l = this.bwidth, h = this.bcolor, f = this.balpha, m = this.gradientRotation, u = this.cornerRadius, p = this.dashLength, s = this.pattern, y = k, w = k;
...
AmCharts.CategoryAxis = AmCharts.Class({inherits: AmCharts.AxisBase, construct: function (a) {
    ...
    if (AmCharts.ifArray(a)) {
        ...
        if (this.parseDates && !this.equalSpacing) {
            ...
            var k = Math.ceil(this.timeDifference / p) + 5, A = f = AmCharts.resetDateToMin(new Date(this.startTime - p), e,
                u, r).getTime();
            ...
            if (this.cellWidth = this.getStepWidth(b), b < k && (k = b), g += this.start, this.stepWidth = this.getStepWidth(b), 0 < k)for (q = Math.floor(b / k), v = this.chooseMinorFrequency(q), h = g, h / 2 == Math.round(h / 2) && h--, 0 > h && (h = 0), k = 0, this.end - h + 1 >= this.autoRotateCount && (this.labelRotation = this.autoRotateAngle), b = h; b <=
                this.end + 2; b++) {
                r = !1;
            ...
            p = h;
            if (this.data[h].time != AmCharts.resetDateToMin(new Date(this.data[h].time), e, u, r).getTime())for (r = 0, B = f, b = h; b < y; b++)m = this.data[b].time, this.checkPeriodChange(e,
        ...
        for (b =
                 0; b < this.data.length; b++)if (n = this.data[b])l = this.parseDates && !this.equalSpacing ? Math.round((n.time - this.startTime) * this.stepWidth + this.cellWidth / 2) : this.getCoordinate(b - g), n.x[this.id] = l;
