DROP FUNCTION IF EXISTS trees.evalnode(integer, numeric, character varying, bigint[], bigint[], character varying, integer, character varying[]);
...
CREATE OR REPLACE FUNCTION trees.evalnode(IN node_id integer, IN patage numeric, IN patsex character varying, IN encsnomeds bigint[], IN probsnomeds bigint[], IN patid character varying, IN customer integer, IN curmedlist character varying[], IN prov_id character varying, IN enc_id character varying)
...
return query execute
    ...
    using patAge, patSex, encSnomeds, probSnomeds, patid, customer, curMedList, prov_id, enc_id;
end;
$BODY$
  LANGUAGE plpgsql VOLATILE
  COST 100
  ROWS 1000;
ALTER FUNCTION trees.evalnode(integer, numeric, character varying, bigint[], bigint[], character varying, integer, character varying[], character varying, character varying)
  OWNER TO maven;