...
class TestAsyncDatabase(unittest.TestCase):
    ...
    def db_main(self, ident, conn):
        ...
        yield from conn.execute_and_close_single("INSERT INTO tomtest" + ident +
                                                 " (num, data) VALUES "+args_str.decode("utf-8"))
        ...
        if not multiple:
            ...
            for cur in cursors:
                for x in cur:
                    res.append(x)
                # cur.close()
                # todo i think there is a bug somewhere but cant find it - >
                # if we run this with cur.close() we get the following error
                # todo psycopg2.ProgrammingError: close cannot be used while
                # an asynchronous query is underway
                # todo it seems to me that the cursors should correspond to
                # different connections and this error should not be happening
                # todo NOTE: this is not always replicable, sometimes it takes
                # a few runs of the unit test to come up
...
class TestBlockingDB(unittest.TestCase):
    """These tests check that the single threaded database connection and query
    work as well as mapping utilites
    """

    RULE_TABLE_MAPPING = ("ruleName", "ruleDescription", "orderType", "orderedCPT",
                          "minAge", "maxAge", "withDX", "withoutDX", "details", "onlyInDept",
                          "notInDept")
    ...
    def testQuery(self):
        ...
        db_blocking.execute_and_close("INSERT INTO ruleTest.rules VALUES ('Rule 1','Description "
                                      + "1','proc',12345,0,200,'11111111','01111111','Details1',"
                                      + "'dept1','dept2');")
        ...
        cursor = db_blocking.execute("SELECT %s FROM ruleTest.rules WHERE orderedCPT "
                                     + "= 12345" % queryParameters, None)
        ...
        db_blocking.execute_and_close("INSERT INTO ruleTest.rules VALUES ('Rule 2','Description 2'"
                                      + ",'proc',12345,0,200,'11111111','01111111','Details2',"
                                      + "'dept1','dept2');")
...
MC.MavenConfig = {
    'test conn pool': {
        ...
        ("dbname=%s user=%s password=%s host=%s port=%s"
         % ('maven', 'maven', 'temporary', 'localhost', '5432')),
